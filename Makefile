# Makefile for BnuThesis

PACKAGE = bnuthesis
THESIS  = bnuthesis-example
SPINE   = spine
...
clean:
	...
	-@$(RM) -rf *~ main-survey.* main-translation.* _markdown_bnuthesis* bnuthesis.markdown.*
...
ifeq ($(version),)
	...
	@[[ $(shell grep -E -c '$(version) Tsinghua University Thesis Template|\\def\\version\{$(version)\}' bnuthesis.dtx) -eq 3 ]] || (echo "update version with \'l3build tag\" before release"; exit 1)
